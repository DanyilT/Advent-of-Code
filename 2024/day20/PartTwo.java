...
public class PartTwo {
        ...
        try {
            ...
            for (int r = 0; r < rows; r++) {
                for (int c = 0; c < cols; c++) {
                    ...
                    List<int[]> reachable = getCellsIgnoringWallsUpToK(grid, r, c, 20);
                    for (int[] pos : reachable) {
                        int rr = pos[0], cc = pos[1], steps = pos[2];
                        if (!isTrack(grid[rr][cc])) continue;
                        if (distE[rr][cc] < 0) continue;
                        int costWithCheat = distS[r][c] + steps + distE[rr][cc];
                        int saved = normalDist - costWithCheat;
                        if (saved >= 100) {
                            cheats.add(String.format("%d,%d %d,%d", r, c, rr, cc));
                        }
                    }
                }
            }
    ...
    private static List<int[]> getCellsIgnoringWallsUpToK(char[][] grid, int sr, int sc, int k) {
        final int[] DIRS = {0,1,0,-1,0};
        int rows = grid.length, cols = grid[0].length;
        ...
        while (!q.isEmpty()) {
            ...
            if (steps > 0 && steps <= k) {
                results.add(new int[]{r, c, steps});
            }
            if (steps == k) continue;
            for (int i = 0; i < 4; i++) {
                int rr = r + DIRS[i], cc = c + DIRS[i+1];
                if (rr < 0 || rr >= rows || cc < 0 || cc >= cols) continue;
                if (!visited[rr][cc][steps+1]) {
                    visited[rr][cc][steps + 1] = true;
