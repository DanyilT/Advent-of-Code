            ...
            for (Map.Entry<String, List<Integer>> report : reports.entrySet()) {
                ...
                if (!isSafeReport(levels)) {
                    ...
                    for (int i = 0; i < levels.size(); i++) {
                        List<Integer> levelsMinusOne = new ArrayList<>(levels);
                        levelsMinusOne.remove(i);
                        if (isSafeReport(levelsMinusOne)) {
                            canBeSafe = true;
                            break;
                        }
                    }
                    safeReports += canBeSafe ? 1 : 0;
                    System.out.printf("The Problem Dampener: %s | Safe report (%b)\n", levels, canBeSafe);
                }
            }
